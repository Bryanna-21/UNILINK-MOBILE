...
// this course; tapping one opens the real detail/result screen at
// app/cat/[id].tsx.
...
export default function CourseCatsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const res = await api.get(`/courses/${courseId}/cats`);
      setCats(res.data?.data ?? []);
    } catch {
      setLoadError('Could not load CATs.');
    } finally {
      setIsLoading(false);
    }
  }, [courseId]);
  ...
  return (
    <View style={styles.container}>
      <StatusBanner status="real" note="CATs are pulled live for this course." />
